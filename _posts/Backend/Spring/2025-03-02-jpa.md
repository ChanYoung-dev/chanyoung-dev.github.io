...
- distnct를 안쓴다면?
  - 아래처럼 나온다
  - ordNo 1 ordDt 20250224 memberNm 김찬영 prdSeq 1 prdId 1542 prdNm 연필 
  - ordNo 1 ordDt 20250224 memberNm 김찬영 prdSeq 2 prdId 1252 prdNm 지우개
  - ordNo 2 ordDt 20250225 memberNm 김찬영 prdSeq 1 prdId 1542 prdNm 연필
  - ordNo 2 ordDt 20250225 memberNm 김찬영 prdSeq 2 prdId 849  prdNm 분필
  - why? fetch join을 하기때문에 즉각적으로 한번에 필요한 데이터를 가져오기때문에 뻥튀기가 된다


- distinct를 쓴다면?
  - 일단 DB단에서 관련된 값들을 전부 조회해옴(물론 이때도 DB에 distinct를 사용함)
...
ToOne관계만 fetch Join하고 나머지는 `hibernate.default_batch_fetch_size`를 통하여 관련된 OeOrdPrd를 여러개 갖고온다
...
<h3>fetch join인데도 불구하고 한번에 되는 이유?</h3>
...
OrderQueryDto에 `List<OrderItemQueryDto>`를 추가로 인자 받는 생성자 추가
```java
public OrderQueryDto(Long orderId, String name, LocalDateTime orderDate, 
